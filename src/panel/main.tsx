...
import { loadMode } from "../state/extMode";
import "../index.css";
import "../chat/styles.css";
import "./panel.css";

// chrome.storage is async, so hydrate settings + the persisted conversation +
// action mode before the first render (mirrors the app's boot pattern).
async function boot() {
  const rootEl = document.getElementById("root");
  if (!rootEl) throw new Error("#root element not found");
  const [settings, conversation, mode] = await Promise.all([
    loadExtSettings(),
    loadConversation(),
    loadMode(),
  ]);
